...
#include "../include/ellipse_detect.h"
#include "../include/template_detect.h"
...
bool checkFishInBlob(cv::Mat frame_gray,cv::KeyPoint kpt,cv::Mat& frame_templDetectOut)
{

    frame_gray.copyTo(frame_templDetectOut);

    cv::Point ptSearch = kpt.pt;
    //Draw A general Region Where the FIsh Is located,
    cv::Point pBound1 = cv::Point(max(0,min(frame_gray.cols,ptSearch.x-gFishBoundBoxSize)), max(0,min(frame_gray.rows,ptSearch.y-gFishBoundBoxSize)));
    cv::Point pBound2 = cv::Point(max(0,min(frame_gray.cols,ptSearch.x+gFishBoundBoxSize)), max(0,min(frame_gray.rows,ptSearch.y+gFishBoundBoxSize)));
    cv::Rect rectFish(pBound1,pBound2);

    cv::rectangle(frame_templDetectOut,rectFish,CV_RGB(20,200,150),2); //Identify Fish Region Bound In Cyan Square

    int bestAngle = 0;
    /// DEtect Fish Features ///
    //Search first Using Fish Model Position/ last position may not have difted far-

    cv::Point ptbcentre;
    int iTemplRow = idxLastTemplateRow;
    int iTemplCol = idxLastTemplateCol;
    double maxMatchScore = doTemplateMatchAroundPoint(frame_gray,
                                                      ptSearch,
                                                      iTemplRow,
                                                      iTemplCol,
                                                      bestAngle,
                                                      ptbcentre,
                                                      frame_templDetectOut);

    cout << "Fish Detected Score :" << maxMatchScore << std::endl;

    //Get Box Around Head point //
    cv::RotatedRect fishRotAnteriorBox(ptbcentre, cv::Size(gLastfishimg_template.cols,gLastfishimg_template.rows), bestAngle);
...
void *camViewEventTrigger(void* tdata){
    ...
    cv::SimpleBlobDetector::Params sblobparams;

    sblobparams.filterByCircularity  = false;
    sblobparams.filterByColor        = false;
    sblobparams.filterByConvexity    = false;
    ...
    // Filter by Area.
    sblobparams.filterByArea = true;
    sblobparams.minArea = 120;
    sblobparams.maxArea = 1200;
    ...
    sblobparams.filterByInertia      = false;
    sblobparams.maxInertiaRatio      = 0.5;

    //params.filterByInertia = true;

    // Set up the detector with default parameters.
    cv::Ptr<cv::SimpleBlobDetector> blobdetector = cv::SimpleBlobDetector::create(sblobparams);
    ...
    //	} else {
      //      filename <<  Reader_input->proc_folder   << '/' << Reader_input->prefix0 << fixedLengthString(ind) << Reader_input->format;
        ...
        if(!image_from_bufferA.empty())
        {
            ...
            blobdetector->detect( fgMaskMOG2, keypoints,gframeMask); //frameMask
            ...
            for(int i=0;i<keypoints.size();i++)
            {
                ...
                if (maskVal > 0)
                {
                     if (checkFishInBlob(image_from_bufferA,kp,frame_templDetectOut) )
                     {
                         keypoints_in_mask.push_back(kp);
                         frame_templDetectOut.copyTo(image_from_bufferA);
                     }
                }
            }
            ...
            // Show Recording CIRCLE Mark On Top LEFT
            if (gbEventRecording)
