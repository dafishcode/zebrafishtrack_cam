...
{
    /// Fix Bounds For Search point such that search template region is not smaller than template size
...
//If We are not Following A known Fish, then Expand the Search Region
    if (iLastKnownGoodTemplateRow ==0 && iLastKnownGoodTemplateCol == 0)
       iSearchRegionSize = 5*gFishBoundBoxSize;
...
int loadTemplatesFromDirectory(QString strDir)
{
    ...
    if (!dirTempl.exists())
    {
        ...
        while (itfile.hasNext())
        {
          ...
          //Save to Glogal List
          vTemplImg.push_back(templFrame);
